...
/* Common functions to check input */
float check_input(float number);
...
float write_coefficient(float coeff){
    ...
    return coeff;
        ...
        if(fabs(check_decimal_part)<=ACCURACY){     /*Comparing float numbers in if statement is not giving correct outputs*/
            if(coeff>0)                            /* Therfore,I converted decimal part into an integer variable to get correct inputs */
                coeff-=decimal_part;               /* ACCURACY =  0.050 * 100000 */
            else if(coeff<0)
...
void sum_of_prime_numbers(){
    ...
    first_number--;             /* initial lower bound */
